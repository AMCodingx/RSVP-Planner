...
import { MapPin, Clock, Phone, Car, Save, ArrowLeft } from 'lucide-react'
...
export function VenueEditPage() {
  ...
  const loadVenue = async () => {
    ...
    setError(null)
    try {
      console.log('Starting venue loading in edit page...')
      const venueData = await venueService.ensureVenueExists()
      console.log('Venue data loaded in edit page:', venueData)
      setVenue(venueData)
      setFormData({
        venueName: {
      ...
      console.error('Failed to load venue in edit page:', err)
      ...
      <div className="flex flex-col items-center justify-center min-h-screen">
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6 max-w-md mx-auto">
          ...
          {error.includes('table does not exist') && (
            <div className="text-sm text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-900/40 rounded p-3 mb-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              Update your wedding venue information and details
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center justify-end space-x-3 p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 rounded-b-lg">
            <button
              type="button"
              onClick={handleBackToVenue}
              className="px-4 py-2 text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg font-medium"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={isSaving}
              className="flex items-center space-x-2 px-6 py-2 bg-rose-500 text-white rounded-lg hover:bg-rose-600 disabled:opacity-50 disabled:cursor-not-allowed font-medium"
            >
              {isSaving ? (
